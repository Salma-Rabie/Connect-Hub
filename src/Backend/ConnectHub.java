...
public class ConnectHub {
    public static void main(String[] args) {
        ...
        // Register handlers
        notificationService.registerListener(new FriendRequestHandler());
        notificationService.registerListener(new GroupActivityHandler());

        // Trigger notifications
        notificationService.notifyListeners("12345", "FriendRequest", "User456 sent you a friend request.");
        notificationService.notifyListeners("12345", "GroupActivity", "New post in 'Programming II Group'.");
         notificationService.notifyListeners("12345", "GroupActivity", "New post in Enginnering group.");

        // Fetch and display notifications
        NotificationManager manager = NotificationManager.getInstance();
        List<Notification> notifications = manager.getUnreadNotifications("user123");
        ...
        if (!notifications.isEmpty()) {
            manager.markAsRead(notifications.get(0).getId());
        }
//        User adminUser = new User.UserBuilder().userId("12345").build();
//        User user1 = new User.UserBuilder().userId("12348").build();
//        User user2 = new User.UserBuilder().userId("12346").build();
//        User user3 = new User.UserBuilder().userId("12347").build();
//
