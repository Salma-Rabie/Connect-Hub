...
import Backend.FriendRequestHandler;
import Backend.NotificationService;
...
// Get received requests as objects
ArrayList<FriendRequestClass> pendingRequests = friendRequestsDatabase.getRequestsReceivedByUserId(user.getUserId());

// Get sent requests as IDs
ArrayList<String> pendingRequestIds = friendRequestsDatabase.getPendingRequests(user.getUserId());

// Extract IDs of users involved in friend requests
HashSet<String> excludedIds = new HashSet<>();

// Add IDs from FriendRequestClass objects
...
excludedIds.addAll(pendingRequestIds);
...
public Object getCellEditorValue() {
    ...
    private void sendFriendRequest(String userId, String friendId) {
        ...
        notificationService.notifyListeners(userId, "FriendRequest",friendId+ " sent you a friend request.");
        } catch (Exception e) {
